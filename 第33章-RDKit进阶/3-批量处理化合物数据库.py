...
# 六、易错点汇总
# ============================================================
# 1. 化合物数据库：从 SDF/CSV/SMILES 文件批量读入
# 2. 大批量用 SDMolSupplier（惰性）+ 逐条处理
# 3. 批量前先验证：有多少条能解析成功、多少失败
# 4. 描述符【预计算并缓存】：
#    同一批分子只算一次，避免重复计算
# 5. 处理要参数化、可复现（固定 seed）
# 6. 结果分批落盘（CSV/SDF），别攒到最后才存

# ============================================================
# 七、自测（40%基础 + 40%中等 + 20%挑战）
# ============================================================
# 【基础】
# 1. 大批量读分子用什么 Supplier？
# 2. 批量处理前要检查什么？
# 3. 描述符为什么要预计算缓存？
#
# 【中等】
# 4. 从 SDF 批量读取分子并统计解析成功率。
# 5. 给一批分子批量算描述符并存成 CSV。
# 6. 处理失败的分子的处理策略。
#
# 【挑战】
# 7. 写一个完整流程：读库 → 清洗 → 批量描述符 → 落盘。
# 8. 解释"预计算缓存"在大规模处理里为什么重要。
