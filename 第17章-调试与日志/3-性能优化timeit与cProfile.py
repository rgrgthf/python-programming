...
# 七、易错点汇总
# ============================================================
# 1. time.time() 和 time.perf_counter()：
#    perf_counter 更精确（计时器），测耗时推荐用后者
# 2. timeit 里引用外部变量，要加 globals=globals()，
#    否则找不到变量
# 3. 优化顺序：先跑对，再测，再优化——别一上来就优化
# 4. 优化前一定要用 cProfile 测出瓶颈，别靠猜
# 5. 集合/字典的 in 是 O(1)，列表 in 是 O(n)，
#    大量查找时先转成 set
# 6. 字符串拼接用 join，别用 +（每次 + 都新建字符串）
...
#
# 【中等】
# 4. 用 timeit 对比列表推导式和生成器表达式的速度。
# 5. 用 cProfile 剖析一个函数，找出最耗时的部分。
# 6. 把慢版的循环过滤改成推导式（参考第五节）。
#
# 【挑战】
# 7. 解释为什么 sum() 比手写循环快？
# 8. 字符串拼接用 join 和 + 的差别，实测对比两者速度。
