...
# 六、易错点汇总
# ============================================================
# 1. combinations 结果【无序】（A,B 和 B,A 算一种）；
#    permutations 结果【有序】（A,B 和 B,A 算两种）
# 2. groupby 只对【连续相同】的相邻项分组，
#    数据要先 sorted 再 groupby，否则分组会乱
# 3. itertools 返回的都是【迭代器】，用完即空，要 list() 保存
# 4. product(repeat=3) 是自身重复三次；product(A, B) 是两个集合
# 5. lru_cache 装饰的函数参数必须可哈希
# 6. partial 固定的是【前几个/指定】参数，注意参数顺序

# ============================================================
# 七、自测（40%基础 + 40%中等 + 20%挑战）
# ============================================================
# 【基础】
# 1. combinations 和 permutations 的区别？
# 2. itertools 的函数返回什么？为什么通常要 list()？
# 3. lru_cache 是干什么的？
#
# 【中等】
# 4. 5 种药两两联用，用 combinations 列出所有组合。
# 5. 用 groupby 分组（注意先排序），输出每组数据。
# 6. 用 partial 固定 write_report 的 author，生成专属函数。
#
# 【挑战】
# 7. 用 product 生成 3 个浓度 × 3 个因素的正交实验设计。
# 8. 用 lru_cache 优化一个递归函数（如斐波那契），
#    并说明为什么它让重复计算变快。
