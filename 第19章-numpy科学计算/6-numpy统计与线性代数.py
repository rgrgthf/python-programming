...
# 八、易错点汇总
# ============================================================
# 1. axis 参数：axis=0 沿【行方向】算（得到每列统计）；
#    axis=1 沿【列方向】算（得到每行统计）——
#    口诀：axis=0 是"跨行"，结果是一列一列的数字
# 2. np.mean/std 默认算【全部元素】的
# 3. np.dot / @ 是矩阵乘法；* 是逐元素乘，别混
# 4. 协方差 np.cov 返回的是矩阵，不是单个值
# 5. 标准化（zscore）= (x - mean) / std：
#    先用它让不同量纲的数据可比
# 6. 线性代数运算前确认数组形状匹配（(n,k)@(k,m)）

# ============================================================
# 九、自测（40%基础 + 40%中等 + 20%挑战）
# ============================================================
# 【基础】
# 1. np.mean(arr, axis=0) 返回什么形状？
# 2. * 和 @ 对矩阵分别做什么？
# 3. 标准差用什么函数？
#
# 【中等】
# 4. 对 3×4 数组分别求 axis=0 和 axis=1 的均值。
# 5. 用矩阵乘法计算两个矩阵的乘积。
# 6. 写 zscore 标准化公式并验证均值 0、标准差 1。
#
# 【挑战】
# 7. 解释 axis=0 和 axis=1 的口诀，举一个实际例子。
# 8. 求一个数组与它自身的协方差矩阵，解释对角线含义。
