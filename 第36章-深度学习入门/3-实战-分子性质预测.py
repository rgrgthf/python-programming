...
# 六、易错点汇总
# ============================================================
# 1. 分子性质预测：输入 = RDKit 描述符，输出 = 性质值
# 2. 特征标准化 + 训练/验证/测试划分（同 QSAR 流程）
# 3. 回归用 MSE/MAE 评估；分类用准确率/AUC
# 4. 小分子数据集（几百条）深度学习优势有限，
#    先和 sklearn 基线对比
# 5. 训练要监视验证损失：过拟合就 early stop
# 6. 把 X 换成真实描述符，模型就成"真实科研工具"
...
#
# 【中等】
# 4. 用描述符训练一个小型神经网络预测性质。
# 5. 和 sklearn 基线对比性能。
# 6. 实现 early stop（验证损失不再降就停）。
#
# 【挑战】
# 7. 用真实 RDKit 描述符做分子性质预测项目。
# 8. 解释为什么小数据要先对比 sklearn。
