...
# 六、易错点汇总
# ============================================================
# 1. seaborn 的数据源要求是 DataFrame（或长表），
#    不是手写 x/y 列表（那是 matplotlib 的活）
# 2. hue="组别" 是 seaborn 的灵魂：按类别分组上色，
#    别自己循环画多条线
# 3. sns 基于 matplotlib：中文/字体设置同样要 rcParams
# 4. 画箱线图/小提琴图前，数据要整理成"长表"（一列类别+一列数值）
# 5. pairplot 数据多会很慢/很大，先抽样再看
# 6. 图例/样式 seaborn 自动管理，比手画省事很多
...
# 1. seaborn 的数据源要求是什么？
# 2. hue 参数的作用？
# 3. seaborn 和 matplotlib 的关系？
#
# 【中等】
# 4. 用 sns 画分组箱线图（组别 → 浓度）。
# 5. 用 sns 画散点图并用 hue 区分组别。
# 6. 用 pairplot 探索多变量数据。
#
# 【挑战】
# 7. 把数据整理成长表，用 sns 画小提琴图+散点叠加。
# 8. 对比 seaborn 和 matplotlib 的适用场景，何时用哪个？
