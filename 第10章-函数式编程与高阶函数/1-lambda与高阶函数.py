...
# 六、易错点汇总
# ============================================================
# 1. lambda 只能写【一行表达式】，不能写多行/if-elif/循环；
#    逻辑复杂就用 def，别硬塞 lambda
# 2. lambda 自带 return：lambda x: x*2 就是返回 x*2，别再写 return
# 3. sorted 不改变原列表，返回新列表；list.sort 才原地改
# 4. sorted(dict) 默认按【键】排；按值排要先 .items() + key
# 5. key 传的是【函数】（不加括号！）key=int 不是 key=int()
# 6. 闭包内部引用外部变量要小心：循环里创建的闭包会"共享"变量
#    （想保留各自的值，用默认参数技巧）
...
# ============================================================
# 【基础】
# 1. lambda 和 def 的区别？什么时候用 lambda？
# 2. sorted 和 list.sort 的区别？
# 3. key 参数传的是什么？（函数本身还是调用？）
#
# 【中等】
# 4. 有数据 [{"药":"A","价":15}, {"药":"B","价":8}]，
#    用 lambda 按价格从高到低排序。
# 5. 解释闭包：make_calibrator(0.985, -0.012) 返回的是什么？
#    它为什么"记得"0.985 和 -0.012？
# 6. max(data, key=lambda d: d["吸光度"]) 返回什么？
#
# 【挑战】
# 7. 用 lambda + sorted 对"药物价格字典"按价格排序输出药名。
# 8. 写一个 make_discount(rate) 闭包：返回"打折函数"，
#    让 make_discount(0.8)(100) == 80。（8折）
