...
# 六、易错点汇总
# ============================================================
# 1. 子结构搜索：mol.HasSubstructMatch(片段)；
#    找所有匹配用 GetSubstructMatches
# 2. 查询片段本身也是一个 Mol（从 SMARTS/SMILES 创建）
# 3. SMARTS 是"模式描述"（比 SMILES 更抽象），
#    能做"含羰基/含苯环"这种通用搜索
# 4. 搜索前确认母分子和片段都解析成功
# 5. 子结构匹配找的是【原子子集】，不是键的子集
# 6. 药效团/骨架搜索都用这个，是药物设计的核心操作

# ============================================================
# 七、自测（40%基础 + 40%中等 + 20%挑战）
# ============================================================
# 【基础】
# 1. HasSubstructMatch 是干嘛的？
# 2. 查询片段怎么创建？
# 3. SMARTS 和 SMILES 的区别？
#
# 【中等】
# 4. 检查布洛芬是否含羧基（SMARTS）。
# 5. 在一批分子里找出含苯环的。
# 6. 用 GetSubstructMatches 找所有匹配位置。
#
# 【挑战】
# 7. 对一批药物分子做子结构筛选（含某个药效团）。
# 8. 解释子结构搜索在药物设计里的用途。
