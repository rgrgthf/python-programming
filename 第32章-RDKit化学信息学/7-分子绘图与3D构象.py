...
# 六、易错点汇总
# ============================================================
# 1. 绘图用 rdkit.Chem.Draw，保存 PNG；
#    中文/复杂结构可能渲染问题
# 2. 3D 构象：AddHs + EmbedMolecule 生成 3D 坐标；
#    构象是"一个可能的结构"，不是唯一的
# 3. SDF 是化学通用格式：SDWriter 写入、SDMolSupplier 读取
# 4. 往 SDF 写属性：mol.SetProp("名称", 值)
# 5. 3D 生成前先加氢（AddHs），否则坐标不完整
# 6. 大批量 SDF 用 SDMolSupplier（惰性读取，省内存）
...
# 1. 分子图用什么模块画？
# 2. SDF 是什么格式？
# 3. 3D 构象生成前为什么要 AddHs？
#
# 【中等】
# 4. 画出一个分子的结构图并保存 PNG。
# 5. 生成一个分子的 3D 构象。
# 6. 用 SDWriter 把分子连同属性写入 SDF。
#
# 【挑战】
# 7. 批量生成一批分子的 SDF（含描述符属性）。
# 8. 解释"构象不唯一"对 3D 建模意味着什么。
