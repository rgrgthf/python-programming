...
# 六、易错点汇总
# ============================================================
# 1. 描述符从 rdkit.Chem.Descriptors 取：
#    MolWt/LogP/HBD/HBA/TPSA 等
# 2. 描述符是【分子性质】的数值化，QSAR 的原料
# 3. 批量算描述符前先确认所有分子解析成功
# 4. 不同描述符量纲不同，建模前要标准化（第35章）
# 5. 描述符算的是"当前 3D/2D 构象"的性质，构象不同值可能略变
# 6. 先算一批，用 pandas 整理成表格，再给 sklearn 用
...
#
# 【中等】
# 4. 计算阿司匹林的 MolWt/LogP/HBD/HBA。
# 5. 对一批分子批量算描述符并整理成 pandas 表格。
# 6. 说明为什么建模前要标准化描述符。
#
# 【挑战】
# 7. 写一个描述符计算函数：SMILES 列表 → DataFrame。
# 8. 解释描述符如何为 QSAR/机器学习提供特征。
