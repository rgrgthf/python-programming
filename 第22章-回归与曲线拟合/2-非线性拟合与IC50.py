...
# 六、易错点汇总
# ============================================================
# 1. curve_fit 需要提供【模型函数】和【初始猜测 p0】，
#    初值给得不好拟合可能发散/不收敛
# 2. IC50 = 抑制率达到 50% 时的浓度，
#    常把浓度取对数后做 S 形（sigmoid）拟合
# 3. 拟合结果看 popt（最优参数）和 pcov（协方差），
#    pcov 对角线开方 = 参数标准差
# 4. 质量控制：看残差是否随机分布、R² 是否合理、
#    参数是否在合理范围
# 5. 数据范围要覆盖 0%~100% 抑制率，否则 IC50 外推不可靠
# 6. 用 scipy.optimize.curve_fit，别自己写最小二乘

# ============================================================
# 七、自测（40%基础 + 40%中等 + 20%挑战）
# ============================================================
# 【基础】
# 1. curve_fit 需要哪两个关键输入？
# 2. IC50 的定义？
# 3. 为什么要对浓度取对数？
#
# 【中等】
# 4. 用 curve_fit 拟合一个简单的线性模型。
# 5. 拟合一个 S 形剂量-反应曲线，提取 IC50。
# 6. 从 pcov 估计参数的不确定性。
#
# 【挑战】
# 7. 写完整的 IC50 拟合脚本：数据 → 拟合 → 提取 IC50 → 出图。
# 8. 解释初值 p0 为什么重要，给错了会怎样？
