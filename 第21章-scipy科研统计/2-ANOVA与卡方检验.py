...
# 五、易错点汇总
# ============================================================
# 1. ANOVA（f_oneway）适合【三组及以上】比较均值；
#    两组用 t 检验
# 2. ANOVA 只告诉你"组间有差异"，但不知道【哪两组不同】；
#    要事后检验（如 Tukey HSD，statsmodels 提供）
# 3. ANOVA 前提：各组正态 + 方差齐性（Levene 检验）
# 4. 卡方检验（chi2_contingency）用于【计数/分类】数据，
#    不是连续数值——别拿它检验均值
# 5. 卡方要求期望频数不过小（通常每个格 >5）
# 6. 输出 p 值前先确认检验方法对应了你的数据类型

# ============================================================
# 六、自测（40%基础 + 40%中等 + 20%挑战）
# ============================================================
# 【基础】
# 1. ANOVA 适合比较几组？和 t 检验怎么分工？
# 2. ANOVA 显著后还需要做什么？
# 3. 卡方检验用于什么类型的数据？
#
# 【中等】
# 4. 对三组数据做单因素 ANOVA，输出 F 和 p。
# 5. 用 Levene 检验检查方差齐性。
# 6. 对 2×2 列联表做卡方检验。
#
# 【挑战】
# 7. 写出完整流程：正态→方差齐→ANOVA→事后检验（Tukey）。
# 8. 说明 ANOVA 和卡方的数据类型区别，各举一个药学例子。
