...
# 八、易错点汇总
# ============================================================
# 1. concat 默认【纵向拼接】（上下加行）；
#    横向拼接要 axis=1（左右加列）
# 2. merge 按【键】合并（类似数据库 join）；
#    concat 是纯拼接。两个别搞混
# 3. to_excel 需要装 openpyxl，否则报错
# 4. to_csv/to_excel 默认会带索引列，
#    导出表格数据通常要 index=False
# 5. 多个 sheet 写入用 pd.ExcelWriter + to_excel(sheet_name=...)
# 6. merge 时注意 on="键"，两个表键名不同用 left_on/right_on

# ============================================================
# 九、自测（40%基础 + 40%中等 + 20%挑战）
# ============================================================
# 【基础】
# 1. concat 和 merge 的区别？
# 2. 导出 CSV 不想带索引列怎么办？
# 3. to_excel 需要什么库？
#
# 【中等】
# 4. 把两个 DataFrame 纵向拼接（忽略原索引）。
# 5. 按"样品ID"把两个表 merge 合并。
# 6. 用一个 ExcelWriter 写两个 sheet（原始数据+汇总）。
#
# 【挑战】
# 7. 写 process_experiment：读 CSV → 分组统计 → 导出 Excel 报告。
# 8. concat(axis=1) 和 merge 的适用场景区别？
