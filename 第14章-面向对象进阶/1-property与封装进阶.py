...
# 八、易错点汇总
# ============================================================
# 1. @property 的方法名和内部变量名要区分：
#    常见陷阱——方法叫 price，内部变量也叫 price 会死循环。
#    所以内部用 _price，property 方法用 price
# 2. 只有 @property 没有 setter → 只读属性，赋值报 AttributeError
# 3. setter 方法名必须和 property 方法【同名】
# 4. @classmethod 的第一个参数是 cls，不是 self，别写错
# 5. __slots__ 后不能加新属性，只适合"属性固定"的大量对象
# 6. 计算属性（amount）不存值，每次访问都重算——别在大量循环里反复访问

# ============================================================
# 九、自测（40%基础 + 40%中等 + 20%挑战）
# ============================================================
# 【基础】
# 1. @property 的作用是什么？外部怎么访问？
# 2. 只读属性怎么做？赋值会怎样？
# 3. 实例方法/类方法/静态方法第一个参数分别是什么？
#
# 【中等】
# 4. 给 Drug 类加 price 属性：负数时报错（用 property + setter）。
# 5. 写一个计算属性：Sample 的 amount = 浓度 × 体积。
# 6. 用 @classmethod 写一个工厂方法 from_dict。
#
# 【挑战】
# 7. 用 __slots__ 定义一个只含两个属性的类，并说明好处和限制。
# 8. 解释：为什么 property 内部要用 _price 而不是直接 price？
#    如果写 price 会出什么问题？
