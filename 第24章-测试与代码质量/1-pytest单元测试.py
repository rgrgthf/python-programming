...
# 五、易错点汇总
# ============================================================
# 1. 测试函数名必须以 test_ 开头，否则 pytest 不会运行它
# 2. assert 失败会报 AssertionError，这就是"测试没通过"；
#    不要在测试里用 print 判断，要用 assert
# 3. 测试文件命名 test_xxx.py，通常和被测代码分开目录
# 4. 一个测试只验证一件事；测试之间不要互相依赖
# 5. fixture 用于准备/清理环境（setup/teardown）
# 6. 改完代码【必跑测试】，防止改坏已有功能（回归测试）

# ============================================================
# 六、自测（40%基础 + 40%中等 + 20%挑战）
# ============================================================
# 【基础】
# 1. 测试函数名为什么要以 test_ 开头？
# 2. 测试失败时会发生什么？怎么判断通过？
# 3. 什么时候跑测试？
#
# 【中等】
# 4. 给一个加法函数写 3 个测试用例。
# 5. 用 assert 断言函数返回值和异常。
# 6. 给测试加 fixture 准备数据。
#
# 【挑战】
# 7. 给 calc_dose 写参数化测试（多组输入输出）。
# 8. 解释回归测试是什么，为什么改代码后必跑。
