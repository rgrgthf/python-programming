...
# 六、易错点汇总
# ============================================================
# 1. QSAR 完整流程：数据 → 描述符 → 划分 → 标准化 → 训练 → 评估
# 2. 描述符【在训练集上算】，不要混入测试集信息（防数据泄漏）
# 3. 评估要看测试集，不看训练集
# 4. 结果要可复现：random_state 固定
# 5. 模型要能和基线（如全猜多数类）对比才有意义
# 6. 这一步做完，就是一个能进作品集的"真实科研作品"
...
#
# 【中等】
# 4. 搭建完整的 QSAR 分类流程（含评估）。
# 5. 固定 random_state 保证可复现。
# 6. 和基线模型对比，说明模型是否有效。
#
# 【挑战】
# 7. 把 QSAR 流程整理成可复用的脚本/函数。
# 8. 解释为什么这个 QSAR 项目能成为复试作品。
