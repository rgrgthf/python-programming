...
# 六、易错点汇总
# ============================================================
# 1. sklearn 三步：建模型 → fit(X_train, y_train) → predict
# 2. 分类评估：accuracy_score / confusion_matrix / classification_report
# 3. 回归评估：r2_score / mean_squared_error
# 4. 特征和标签都要是【数值】；字符串要先编码
# 5. 数据要标准化（StandardScaler），否则量纲影响模型
#    （注意：scaler 只 fit 训练集，再 transform 测试集）
# 6. 新分子预测：和训练时【同样预处理】后 predict
...
# 4. 训练一个分类模型并评估准确率。
# 5. 训练一个回归模型并看 R²。
# 6. 说明 scaler 为什么只 fit 训练集。
#
# 【挑战】
# 7. 用训练好的模型预测几个新分子。
# 8. 解释标准化对模型的影响，举一个量纲例子。
